...
const Profile = ({ toggleProfile, showProfile, cpuOnly }) => {
  return (
    <div
      ...
      } flex items-center justify-center absolute top-0 left-0 w-screen h-screen duration-500`}
    >
      <div
        className={`${
          showProfile ? "translate-y-0 opacity-100" : "-translate-y-6 opacity-0"
        } z-[52] rounded-3xl shadow-xl dark:shadow-indigo-900/40 max-w-sm m-3 dark:bg-black/95 dark:text-white bg-white/95 text-black transition-all duration-500`}
      >
        <button
          onClick={() => {
            toggleProfile();
        ...
        <div className="text-center px-4 pb-6 pt-2">
          ...
          <p className="mt-2 font-light opacity-70 text-sm sm:text-base">
      ...
      <div
        ...
        className={`${showProfile ? "opacity-100" : "opacity-0"} ${
          cpuOnly
            ...
            : "backdrop-blur bg-gray-50/70 dark:bg-black/70"
        } absolute w-screen h-screen transition-all duration-500 top-0 left-0 z-[51]`}
      ></div>
    </div>
  );
};
